...
def get_bins(v):
    ...
    if v < 60:    # Grade F
        return 5
    elif v >= 60 and v < 70:  # Grade D
        return 6
    elif v >= 70 and v < 80:  # Grade C
        return 7
    elif v >= 80 and v < 90:  # Grade B
        return 8
    elif v >= 90:  # Grade A
        return 9

def get_technology_accuracy(weights, max_score, tech):
    '''Function where you pass weights, max score and fundamental data to calculate final score and returns the accuracy to find
       the optimal weight in genetic modelling'''
    t1 = pd.DataFrame()
...
tech = d5[d5['type'] == 1]
d1 = d1[d1['date'] == d1['date'].sort_values().iloc[-30]]
...
# Fitness function to optimize the accuracy
...
if '__main__' == __name__:
    ...
    res.to_csv('/home/tokenmetrics/data/fundamental/New_roi.csv')
    x.to_csv('/home/tokenmetrics/data/fundamental/Old_roi.csv') 
    pd.DataFrame([pd.Series(tech['question_number'].unique()).sort_values().tolist(), w]).to_csv('/home/tokenmetrics/data/fundamental/weights_roi.csv', index=False)
    new_grade[['Coin','Score']].to_csv('/home/tokenmetrics/data/fundamental/new_grade_roi.csv')
    ...
    y.to_csv('/home/tokenmetrics/data/fundamental/alpha_roi_roi.csv')