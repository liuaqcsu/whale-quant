...
d1 = d1[d1['date'] == datetime.date(2019,7,31)]

t2 = pd.DataFrame()
for coin in df['ico_symbol'].unique().tolist():
    t1 = df[df['ico_symbol'] == coin]
    t1 = t1[t1['date'] >= datetime.date(2019,7,31)][t1['date'] <= datetime.date(2019,8,30)]
...
def fitness(indv):
    ...
    res = get_result(w)
    res = (res['Total'] * res['% Accurate'] / 100)
    res = res[0] + res[1] + res[3] + res[4] + res[2]
    res = float(abs(res))
    #print(w,': ',res)
    return res
