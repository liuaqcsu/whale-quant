...
def plot_returns_bars(returns, benchmark=None,
    ...
    if isinstance(benchmark, _pd.Series):
        df[c2] = benchmark[benchmark.index.isin(returns.index)]
        df = df[[c2, returns_label]]
    ...
    df.to_csv('/home/tokenmetrics/data/quant/temp/'+title+'.csv')
...
def plot_timeseries(returns, benchmark=None,
                    ...
                    fill=False, returns_label="Strategy",c2='BTC',
                    hline=None, hlw=None, hlcolor="red", hllabel="",
    ...
    df.to_csv('/home/tokenmetrics/data/quant/temp/'+title+'.csv')
    if isinstance(benchmark, _pd.Series):
        ax.plot(benchmark, lw=lw, ls=ls, label=c2, color=colors[0])
...
def plot_rolling_stats(returns, benchmark=None, title="",
    ...
    if isinstance(benchmark, _pd.Series):
        ...
        ax.plot(df['Benchmark'], lw=lw, label=c2,
...
def plot_rolling_beta(returns, benchmark,
    ...
    df = _pd.DataFrame()
    df['beta1'] = beta
    df['beta2'] = _stats.rolling_greeks(returns, benchmark, window2)['beta']
    df.to_csv('/home/tokenmetrics/data/quant/temp/'+title+'.csv')
...
def plot_longest_drawdowns(returns, periods=5, lw=1.5,
    ...
    for idx, row in longest_dd.iterrows():
        ...
        t0 = "{} {}".format(*_mdates.datestr2num([str(row['start']), str(row['end'])]))
        temp.append(t0)
        #t0.append(temp[0])
        #t1.append(temp[1])
    ...
    df = _pd.DataFrame()
    print(len(series),len(temp))
    df['backtest'] = series
    df['axvspan'] = _pd.Series(temp)
    df.to_csv('/home/tokenmetrics/data/quant/temp/'+'Drawdown Periods'+'.csv')
...
def plot_distribution(returns, figsize=(10, 6),
    ...
    df = port
    df.to_csv('/home/tokenmetrics/data/quant/temp/'+'Quantile Returns'+'.csv')
